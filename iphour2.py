...
def thread_get_dollar_conversion():
    ...
    Using Free Currency Convert (FCC) as first option, 
    then ExchangeRate API (ERA) as fallback. 
    First updates every hour, the other once every day. 
    We are prioritizing FCC over ERA. 
    '''
    while True:
        ...
        try:
            ...
            print("thr3_dollarconv got an update from FCC")
            time.sleep(3600) 
        except:
            ...
            print("thr3_dollarconv got an update from ERA")
            time.sleep(3600) 
        finally:
